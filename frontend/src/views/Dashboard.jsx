  ...
  useEffect(() => {
    const getHeatMapData = async () => {
      ...
      const response = await axios.get(
        `http://localhost:5000/api/blog/contributions/${userid}`
      );
    ...
    if (user) {
      ...
      if (user.lcUsername) {
        getLeetcodeProfile(user.lcUsername);
      }
    } else {
      console.log("no user obj found");
    }
  }, []);
          ...
          <div className="w-2/12 mr-6">
            ...
            <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-b-xl shadow-lg px-6 py-6 space-y-4">
              ...
              <StatRow
                ...
                label="Total Blogs"
                value={`${user?.blogs ? user?.blogs?.length : 0}`}
              />
              <StatRow
                icon={<IconEye size={22} />}
                label="Profile Views"
                value={`${user?.views ? Object.keys(user?.views).length : 0}`}
              />
            </div>
          </div>
          ...
          <div className="w-10/12">
            <div className="flex flex-row">
              <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl w-7/12 mr-2 p-6">
                <p className="text-4xl text-[#5eead4] mb-1">{greeting},</p>
              ...
              <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl shadow-lg p-6 w-full text-white overflow-auto scrollbar-hide">
                ...
                <div className="flex gap-4 items-center mb-6">
                  ...
                  <div className="flex gap-3">
                    {leetcodedata?.badges?.badges?.length > 0 ? (
                    ...
                    <div className="flex flex-col space-y-4 text-md">
                      <div className="flex justify-between items-center font-bold">
                        ...
                        <span className="mx-1 text-yellow-500">
                          {leetcodedata?.contest?.totalattended ?? "N/A"}
                        </span>
                      </div>
                      <div className="flex justify-between items-center font-bold">
                        <p>Rating</p>
                        <span className="mx-1 text-yellow-500">
                          {leetcodedata?.contest?.contestRating ?? "N/A"}
                        </span>
                      </div>
                      <div className="flex justify-between items-center font-bold">
                        <p>Global Rank</p>
                        <span className="mx-1 text-yellow-500">
                          {leetcodedata?.contest?.contestGlobalRanking ?? "N/A"}
                        </span>
                      </div>
